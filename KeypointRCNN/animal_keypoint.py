    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Dict]:
        ...
        targets = {
            ...
            'keypoints': torch.as_tensor(
                np.concatenate([targets['keypoints'], np.ones((8, 1))], axis=1)[np.newaxis], dtype=torch.float32        # pig
                # np.concatenate([targets['keypoints'], np.ones((13, 1))], axis=1)[np.newaxis], dtype=torch.float32     # cow
            )
        }
